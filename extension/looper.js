...
// feature extract
  ...
  function extractFeatures(samples, sampleRate, opts = {}) {
    ...
    for (let fIdx = 0; fIdx < nFrames; fIdx++) {
      ...
      // log-compress, mean-subtract, l2-normalize: similarity is volume-invariant, so fading repeats still match
      let mean = 0;
...
// lag search
  ...
  async function findBestLag(feat, opts = {}, onProgress) {
    ...
    const thr = opts.simThreshold ?? 0.80;
    ...
    for (let lag = minLag; lag <= maxLag; lag++) {
      ...
      for (let t = 0; t < M; t++) {
        ...
        if (sA && sB) { // both silent: neutral, extends a run without evidence
          if (inRun && ++neutral > maxSilenceBridge) closeRun();
          continue;
        }
        if (sA || sB) { // silence vs music: mismatch
      ...
      if ((lag - minLag) % 64 === 0) {
        if (onProgress) onProgress((lag - minLag) / (maxLag - minLag));
        await yieldFn();
      }
    }
...
// pipeline
  ...
  async function analyzeSamples(samples, sampleRate, opts = {}, onProgress) {
    ...
    await (opts.yieldFn || defaultYield)();
    ...
    if (!best) {
      return {
        found: false,
        reason: 'No sustained self-repetition found. The file may contain only a single pass of the loop (nothing to superimpose), or the loop may be shorter than the minimum loop length setting.',
    ...
    // one frame inside the matched run: the intro can't bleed into the loop
